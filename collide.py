...
def doit(outdir, args):
    ...
    for i, fn in enumerate([args.file1, args.file2]):
        logger.debug("rendering file %d..." % (i+1))
        subprocess.check_call(['gs', '-r%d' % args.resolution, '-sDEVICE=png16m',
            '-o', os.path.join(outdir, 't%d-%%04d.png' % (i+1)), fn],stdout=FNULL,stderr=subprocess.STDOUT)
    ...
    subprocess.check_call(['cjpeg', '-quality', str(args.quality)] + j1enc + [
        ...
        '-optimize', '-verbose', '-targa', os.path.join(outdir, 'p1.tga')],stdout=FNULL,stderr=subprocess.STDOUT)
    ...
    subprocess.check_call(['cjpeg', '-quality', str(args.quality),
        ...
        '-optimize', '-verbose', '-targa', os.path.join(outdir, 'p2.tga')],stdout=FNULL,stderr=subprocess.STDOUT)
    ...
    # render PDFs
    logger.debug("producing final PDFs")
    pdf = PDFGenerator()
